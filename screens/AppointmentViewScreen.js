...
class AppointmentViewScreen extends React.Component {
    static navigationOptions = ({ navigation }) => {
        const slot = navigation.getParam('slot', '');
        return {
            headerTitleStyle: {
                        ...
                        <MenuTrigger>
                            <View style={{ padding: 5 }}>
                                <Icon name="ellipsis-v"
                                    type={"regular"}
                                    size={20}
                                    color={"#888888"} marginLeft={28} style={{ padding: 100 }} />
                            </View>
                        </MenuTrigger>
                            ...
                            <MenuOption
                                onSelect={() => {
                                    Alert.alert(
                                        ...
                                        {
                                            text: 'OK',
                                            onPress: () => { navigation.state.params.deleteAptIdCallback(slot.slotid); navigation.pop(1); }
    ...
    async componentDidMount() {
        ...
        //console.log("serviceColor", this.props.navigation.getParam('slot').serviceColor)
        //console.log("SLOTONVIEWCUSTOMENAME", this.props.navigation.getParam('slot').customer)

        if (this.props.navigation.getParam('slot', '')) {
            this.setState({
                slot: this.props.navigation.getParam('slot', ''),
            })
        }

        //const selectedEmpName = this.props.employees[this.props.navigation.getParam('slot').employeeids]
        //console.log("selected emp", selectedEmpName.nickname)
        // this.setState({
        //     selectedEmpName: this.state.selectedEmpName
        // })

        //const selectedServiceName = this.props.services[this.props.navigation.getParam('slot').serviceid]
        //console.log("selected service", selectedServiceName.name)
        // this.setState({
    ...
    render() {
        ...
        const slot = navigation.getParam('slot', '');
        const selectedEmpName = this.props.employees[this.props.navigation.getParam('slot').employeeids]
        //console.log("2New1111selectedEmpNamerender", selectedEmpName.user.first_name);
        const selectedServiceName = this.props.services[this.props.navigation.getParam('slot').serviceid]
        //console.log("66newselected service", selectedServiceName.name)
        //console.log("1111selectedEmpNamerender", this.props.employees[this.props.navigation.getParam('slot').employeeids].nickname);
        //console.log("222selectedSERVICES", this.state.selectedServiceName.name)
        const topRowHeight = Layout.isSmallDevice ? 460 : 540;
        const bottomRowHeight = Layout.isSmallDevice ? 60 : 80;
        const { serviceSelectModalVisible } = this.state;
                    ...
                    <View style={{
                        ...
                        <View style={styles.rowStyle1}>
                            ...
                            <View style={{
                                ...
                                {/* {selectedEmpName.user.first_name != null ? <Text style={{
                                    fontFamily: 'open-sans-hebrew',
                                    fontSize: 25,
                                    color: '#000000'
                                }}>
                                    {selectedServiceName.user.first_name}
                                </Text> :  */}
                                <Text style={{
                                    fontFamily: 'open-sans-hebrew',
                                    fontSize: 25,
                        ...
                        <View style={styles.rowStyle1}>
                            ...
                            <View style={{
                                ...
                                <Text style={{
                                    ...
                                    fontSize: 16,
                                    color: '#AAAAAA'
                                }}>{moment(slot.startTime, "YYYY-MM-DD HH:mm:ss").format("HH:mm, DD/MM/YYYY")}
                                    {(this.checkSameDay(slot.startTime)) ? ' (Today)' : ''}</Text>
                                {/* : <Text
                                        style={styles.textStyle5}>{moment(dateTime, "YYYY-MM-DD HH:mm:ss").format("HH:mm, DD/MM/YYYY")}
                                            {(this.checkSameDay(dateTime)) ? ' (Today)' : ''}
                                            </Text>} */}
                        ...
                        <View style={styles.rowStyle1}>
                            ...
                            <View style={{
                                ...
                                {selectedEmpName.user.first_name != null ? < Text style={{
                                    ...
                                    fontSize: 16,
                                    color: '#AAAAAA'
                                }}>{selectedEmpName.user.first_name}</Text> :
                                    < Text style={{
                                        fontFamily: 'open-sans-hebrew',
                                        fontSize: 16,
                                        color: '#AAAAAA'
                                    }}>{selectedEmpName.nickname}</Text>}
                                {/* // : <Text style={styles.textStyle5}>{employeeName}</Text>} */}
                            </View>
                        </View>
                        {/* </TouchableWithoutFeedback> */}
                    </View>
                    <View style={{
                        ...
                        <View style={styles.rowStyle1}>
                            ...
                            <View style={{
                                ...
                                {/* : <Text style={styles.textStyle5}>{customerName}</Text>} */}
                            </View>
                        </View>
                        {/* </TouchableWithoutFeedback> */}
                    </View>
...
export default connect(mapStateToProps, mapDispatchToProps)(AppointmentViewScreen)
